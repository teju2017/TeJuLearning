...
####################################################################################
# Script name  : data_cloning.pl
# Author       : Thejas HM
# Creatio dt   : 03-AUG-2018
# Description  : Script to enable the cloning of the consultation id provided
#                it displays the data of the consultation id provided and loads 
#                with a new id
###################################################################################
...
sub data_get_values()
{
    ...
    while(@data = $sth->fetchrow_array())
    {
       ...
       if($record_count == 0 )
       {
           ...
           system('/sbclocal/gdmp/web/cgi-bin/message_display.pl',"NO RECORD FOUND FOR THE CONSULTATION ID $con_id to clone, check in the search page");
       }
           foreach $x (@data)
           {
             $counter=$counter+1;
    ...
    ####### THIS LOGIC TO CHECK WHETHER THE CONSULTATION ID EXISTS OR NOT #####################
...
sub data_edit_display()
{
   ...
   print "<head>
           ...
           <h1>ONBOARDING DETAILS - CLONING</h1>
...
<div class=insert>
<div>
<form action=/cgi-bin/data_cloning_insert.pl method=GET>
<div class=Core>
...
$consultation_id=$obj->param('cloningid');
